...
| **사이드 메뉴** | 카테고리 필터 + 서비스 개수 표시 + 누적 방문 수 |
...
- **방문자 수 카운터** — 사이드바 하단에 누적 방문 수 표시 (hits.seeyoufarm.com API, 가입·설정 불필요)
...
- 방문자 카운터 — hits.seeyoufarm.com API (가입·설정 불필요)
- GitHub Pages 배포
...
| 2026-06-11 | **방문자 수 확인 불가** | 사이드바 하단 누적 방문 수 카운터 추가 (hits.seeyoufarm.com, 가입 불필요) |
...
| 중간 | **방문자 카운터 외부 API 의존** | hits.seeyoufarm.com 장애 시 방문 수 미표시(`-`로 폴백) |
